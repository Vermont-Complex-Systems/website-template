<script>
    ...
    // DuckDB for global stats + district aggregation
    const db = database(
    ...
    // All DA centroids for district-level aggregation
    ...
    // Fetch DAs when a district is selected
    $effect(() => {
        ...
        const [[minX, minY], [maxX, maxY]] = d3.geoBounds(district);
            ...
            try {
                ...
                for await (const feature of iter) {
                    ...
                    if (feature.properties.population > 0) {
                        ...
                        rewound.properties.pop_density = rewound.properties.population / (rewound.properties.area_sqkm || 1);
                        features.push(rewound);
                    }
                }
                if (!cancelled) {
                    daFeatures = features;
    ...
    <h3>
        {#if isZoomed}
            ...
            {:else}
                ...
                <span class="timing">({fetchTime}ms &middot; {normalizeGlobal ? 'city-wide' : 'district'} scale)</span>
            {/if}
        {:else}
            Click a district to zoom in <span class="timing">(FlatGeobuf)</span>
        {/if}
    </h3>
